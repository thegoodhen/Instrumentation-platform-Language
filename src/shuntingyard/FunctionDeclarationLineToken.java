	...
	public void prepare(Compiler c) {
		...
		if (matcher.matches()) {
			...
			for (String s : splitArgs) {
				...
				if (gvdlt == null) {
					System.err.println("Error: Invalid function argument: \"" + s + "\" !");
				} else {
					gvdlt.prepare(c);
...
//And now we try matching the ENDIF
		...
		for (int i = lineNumber + 1; i < c.getLines().size(); i++) {
			...
			if (t instanceof FunctionDeclarationLineToken) {
				System.err.println("Error! Function declaration nesting is not supported; expected ENDFUNCTION before new function declaration!");
				break;
		...
		if (endToken == null) {
			System.err.println("Attempt to find a matching ENDFUNCTION failed; Reached end of code while parsing.");
		}
	}
