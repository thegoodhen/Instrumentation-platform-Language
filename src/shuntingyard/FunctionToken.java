...
public class FunctionToken extends CompilableToken {
     ...
     * @param c
     */
    protected void doLookup(Compiler c) throws CompilerException {

	FunctionToken ft = c.getFunction(this.tokenString);
	if (ft == null) {
	    throw new CompilerException("Error: Attempt to call an unknown function: \"" + this.getTokenString() + "\" !");
	} else {
	...
	for (int i = this.getArgumentCount() - 1; i >= 0; i--) {
		...
		if (((NumberToken) currentParameter).isImplicitlyCastableTo(argumentList.get(i).getType())) {
		 startingPositionOfArgumentOnStack+=   ((NumberToken) currentParameter).compileCastTo(argumentList.get(i).getType(), startingPositionOfArgumentOnStack - 1, c);
		} else {

		    throw new CompilerException("Cannot implicitly cast " + ((NumberToken) currentParameter).getShortName() + " into " + argumentList.get(i).getType() + "; this might lead to a lossy conversion, however the function call isn't valid without this illegal conversion.");
		}
		...
		throw new CompilerException("Not enough arguments for the function " + this.getTokenString());
	    }
	}
