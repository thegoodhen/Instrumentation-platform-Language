...
public class Compiler {
	...
	FunctionToken prevF = functionMap.get(f.getTokenString());
	if (prevF != null)//function is already declared!
	{
	    if (prevF.getDeclaration() != null && prevF.getDeclaration().getJumpTarget() != null)//recompilation!
	    {
		int posInBytecode = functionByteCode.indexOf(prevF.getDeclaration().getJumpTarget());
		if (posInBytecode != -1) {
		    System.out.println("Recompiling previously defined function: " + prevF.getTokenString());
		    f.getDeclaration().jtbct = prevF.getDeclaration().getJumpTarget();//this should retarget everything nice and smooth; not sure if necessary
		    for (int i = posInBytecode; i >= 0; i--) {
			Token currentToken = functionByteCode.get(i);
			if (currentToken instanceof EndFunctionLineToken) {
		    ...
		    System.out.println("Warning, overwrote a function, which was duplicitely defined: " + prevF.getTokenString());
		}
